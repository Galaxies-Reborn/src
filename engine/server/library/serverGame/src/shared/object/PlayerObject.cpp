...
{
	// Publish 14 uses faction standing and creature faction rank, not the
	// post-NGE passive regional score derived from combat level and weekly rank.
	// Preserve this compatibility entrypoint and the regional presentation data,
	// but do not mutate the later-era GCW score system.
	UNREF(gcwCategory);
	UNREF(po);
	UNREF(co);
}
